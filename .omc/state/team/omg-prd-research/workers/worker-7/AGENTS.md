...
```bash
mkdir -p $(dirname .omc/state/team/omg-prd-research/workers/worker-7/.ready) && touch .omc/state/team/omg-prd-research/workers/worker-7/.ready
```
...
- **Worker**: worker-7
- **Agent Type**: codex
- **Environment**: OMC_TEAM_WORKER=omg-prd-research/worker-7
...
2. Update status to "in_progress", set owner to "worker-7"
3. Write back to task file
4. Do the work
5. Update status to "completed", write result to task file

## Communication Protocol
- **Inbox**: Read .omc/state/team/omg-prd-research/workers/worker-7/inbox.md for new instructions
- **Heartbeat**: Update .omc/state/team/omg-prd-research/workers/worker-7/heartbeat.json every few minutes:
  ```json
  {"workerName":"worker-7","status":"working","updatedAt":"<ISO timestamp>","currentTaskId":"<id or null>"}
  ```

## Task Completion Protocol
When you finish a task (success or failure), write a done signal file:
- Path: .omc/state/team/omg-prd-research/workers/worker-7/done.json
...
2. Write an ACK file: .omc/state/team/omg-prd-research/workers/worker-7/shutdown-ack.json
3. Exit
